...
| `/analyze` | 两个入口:① 粘贴聊天记录 ② 上传音频(录音 ≤10 分钟);录音前知情提示 |
...
| **M2** | 接 OpenAI Whisper 转文字 + **音频用后即删** | 10 分钟音频 < 60 秒出文字;服务器不留存音频文件 |
...
2. **音频即删不存**:转文字完成后立即删除音频;不做任何音频分享/公开/存储功能。
